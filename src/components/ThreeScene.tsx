...
function ThreeScene() {
  ...
  function createMarkersFromGeoJson(geoJson, scene, radius) {
  ...
  function handleCameraMove(lat, lon) {
    if (!cameraRef.current || !controlsRef.current) return;

    const targetPosition = latLongToVector3(lat, lon, 5); // Assuming a globe radius of 5
  ...
  useEffect(() => {
    ...
    // const pivotHelper = new THREE.AxesHelper(10); // Helps visualize the pivot position
    ...
    const geoLoader = new THREE.FileLoader();
    geoLoader.load("custom.geo.json", function (data) {
